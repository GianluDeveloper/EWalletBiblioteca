...
public class MovimentiDao {
	...
	public void update(Movimenti d) throws Exception {
		Object[] campi = {d.getIban(),d.getIdTipoMovimento(),d.getImporto(),DateHandler.toSql(d.getDataMovimento()),d.getId()};
	...
	public List<Movimenti> findAll(boolean reverse) throws Exception {
		...
		String sql = "SELECT `id`, `iban`, `idTipoMovimento`, `importo`, `dataMovimento` FROM `movimenti` WHERE ? ORDER BY id "+sort;
		DBHandler dbHandler = new DBHandler();
		dbHandler.sql(sql, campi);
		List<Object>objs = dbHandler.getResponse();
		for(Object obj:objs) {
